...
class CustomTripletMarginLoss(nn.Module):
    ...
    def forward(self, anchor, positive, negative):
        ...
        cos_sim_neg = F.cosine_similarity(anchor, negative, dim=1)

        # 使用余弦距离计算损失
        loss = F.relu(self.margin - cos_sim_pos + cos_sim_neg).mean()

        return loss

criterion_triplet = CustomTripletMarginLoss(margin=0.1)
...
def inter_intra_corration_loss(category_corration, margin=0.1):
    ...
    image_specific_prototype = category_corration[0].view(N, -1)   # N, 64, W, H -->> N, 64, WH

    G1_image_specific_prototype = image_specific_prototype[:N // 2, :]
    G2_image_specific_prototype = image_specific_prototype[N // 2:, :]
    # G3_image_specific_prototype = image_specific_prototype[2*N // 3:, :]    # N // 3, 64, WH

    G1_consensus_prototype = category_corration[1][0].view(N // 2, -1)  # N // 3, 64, W, H -->> N // 3, 64, WH
    G2_consensus_prototype = category_corration[1][1].view(N // 2, -1)
    # G3_consensus_prototype = category_corration[1][2].view(N // 2, -1)
    corration_loss1 = criterion_triplet(G1_consensus_prototype, G1_image_specific_prototype, G2_consensus_prototype)

    corration_loss2 = criterion_triplet(G2_consensus_prototype, G2_image_specific_prototype, G1_consensus_prototype)

    return corration_loss1 + corration_loss2
