...
def main(cfg):
    ...
    dataset_names = cfg.datasets.split('+')
    for dataset in dataset_names:
        loader = EvalDataset(os.path.join(cfg.pred_dir, dataset), '/test_data/' + dataset +'/gt/')
        print('Evaluating predictions from {}'.format(os.path.join(cfg.pred_dir, cfg.methods, dataset)))
        thread = Eval_thread(loader, cfg.methods, dataset, cfg.output_dir, cfg.cuda)
        info, continue_eval = thread.run(continue_eval=True)
        print(info)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--methods', type=str, default='Sg-CoSOD')
    ...
    parser.add_argument('--pred_dir', type=str, default='../test_result/', help='predictions')
