...
class DSLoss(nn.Module):
    ...
    def __init__(self):
        ...
        if 'triplet' in self.lambdas_sal_last and self.lambdas_sal_last['triplet']:
            margin = self.config.triplet_loss_margin  # 0.1
    ...
    def forward(self, scaled_preds, gt, norm_features=None, labels=None): #scaled_preds, gts, norm_features=norm_features, labels=cls_gts
        loss = 0
